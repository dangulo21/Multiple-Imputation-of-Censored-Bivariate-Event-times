...
for(l in L){
  path = paste0(getwd(),"/results/",cens_type,"/",method,"/L",l,"-M20")
  ...
  for(file in files){
    ...
    results_combine <-
      results_combine |>
      group_by(
        correlation, censoring, n, t1, t2, L,
      ...
      summarize(
        ...
        #Others
        sims.na.T12 = sum(is.na(coverage.loglog.T12)),
        sims.na.T1 = sum(is.na(coverage.loglog.T1)),
        sims.na.T2 = sum(is.na(coverage.loglog.T2)),
        sims.total = n())
...
path = paste0(getwd(),"/results/",cens_type,"/",method)
files = list.files(path)
...
results$L <- paste0("L-",results$L)
df <- rbind(results,results2)


save(df, file = paste0(getwd(),"/results/",cens_type,"/results-combine.RData"))
