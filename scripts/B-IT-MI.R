...
library(MIWilson)
  ...
  #boot.MI() is a function that returns a data.frame with estimates survival probabilities
  #for the b-th bootstrap sample. Such survival probabilities are esitmates across a common grid of values (t1,t2)
  #located in df_comp.
  
  boot.MI <- function(df_sim, boot){
    ...
    while(iter <= (L)*2){
      ...
      if(iter > L*2){
        ...
        # For that completed data set (df_imp), which is the b-th bootstrapped sample
        # get the estimated probabilities across the grid of values (df_iter/df_comp). 
        ...
        # return the estimated probabilities for the b-th bootstrap sample
        return(list(df_iter, avg.epsilon))
  ...
  # The following loop creates each of the M bootstrap samples, calls for boot.MI()
  ...
  for(boot in 1:M){
    ...
    data_boot <- data_sim[index,]
    
    # The maximum observed time should be an event (to make sure we have correct imputations)
    max.t1.delta = ifelse(TRUE %in% data_boot[which(data_boot$x1 == max(data_sim$x1)),]$delta1, TRUE, FALSE)
    max.t2.delta = ifelse(TRUE %in% data_boot[which(data_boot$x2 == max(data_sim$x2)),]$delta2, TRUE, FALSE)
    
    # Find the seed such that the maximum observed time for T1 and T2 is an event.
    while( (max.t1.delta  & max.t2.delta) != TRUE){
      index <- sample(seq(1,n), n, replace = TRUE)
      data_boot <- data_sim[index,]
    ...
    res <- boot.MI(data_boot, boot)
    df_mi <- rbind(df_mi, res[[1]])
    avg.epsilon <- c(avg.epsilon, res[[2]])
    remove(res)
  }
